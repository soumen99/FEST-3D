        ...
        select case(trim(scheme%scalar_transport))
          case('grad_diffusion')
            ...
            if(kmx==2)then
                ...
                call compute_diffusion_fluxes_scalar(H, qp, cells, Kfaces, flagsk, dims)
            end if
            case('none')
                !do nothing
              continue
      ...
      do k = 1, dims%kmx - 1 + kk
       do j = 1, dims%jmx - 1 + jj
        do i = 1, dims%imx - 1 + ii
          ...
          ! adding viscous fluxes to stored convective flux
          F(i, j, k, 8) = F(i, j, k, 8) - (area*((mu_f + mut_f)*(dtgmdx*nx + dtgmdy*ny + dtgmdz*nz)))
      ...
      do k = 1, dims%kmx - 1 + kk
       do j = 1, dims%jmx - 1 + jj
        do i = 1, dims%imx - 1 + ii
          ...
          ! difference in state across face
          delphi = qp(i, j, k, dims%n_var) - qp(i-ii, j-jj, k-kk, dims%n_var)
