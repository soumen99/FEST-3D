...
!              qp(i,j,k,7) = conservativeQ(7) / conservativeQ(1)
    ...
    subroutine update_scalar_variables(qp, residue, delta_t, cells, Ifaces, Jfaces, Kfaces, dims)
      ...
      !< Input varaible which stores K faces' area and unit normal
      integer :: i,j,k
        real(wp), dimension(1:9)     :: deltaU
        real(wp), dimension(1:9)     :: D
        real(wp), dimension(1:9)     :: conservativeQ
        real(wp), dimension(1:9)     :: OldIminusFlux
        real(wp), dimension(1:9)     :: OldJminusFlux
        real(wp), dimension(1:9)     :: OldKminusFlux
        real(wp), dimension(1:9)     :: NewIminusFlux
        real(wp), dimension(1:9)     :: NewJminusFlux
        real(wp), dimension(1:9)     :: NewKminusFlux
        real(wp), dimension(1:9)     :: DelIminusFlux
        real(wp), dimension(1:9)     :: DelJminusFlux
        real(wp), dimension(1:9)     :: DelKminusFlux
        real(wp), dimension(1:6)     :: LambdaTimesArea
        real(wp), dimension(1:9)     :: Q0 ! state at cell
        real(wp), dimension(1:9)     :: Q1 ! state at neighbours
        real(wp), dimension(1:9)     :: Q2
        real(wp), dimension(1:9)     :: Q3
        real(wp), dimension(1:9)     :: Q4
        real(wp), dimension(1:9)     :: Q5
        real(wp), dimension(1:9)     :: Q6
        real(wp), dimension(1:9)     :: DQ0! change in state
        real(wp), dimension(1:9)     :: DQ1
        real(wp), dimension(1:9)     :: DQ2
        real(wp), dimension(1:9)     :: DQ3
        real(wp), dimension(1:9)     :: DQ4
        real(wp), dimension(1:9)     :: DQ5
        real(wp), dimension(1:9)     :: DQ6
        real(wp), dimension(1:8)     :: Flist1
        real(wp), dimension(1:8)     :: Flist2
        real(wp), dimension(1:8)     :: Flist3
        real(wp), dimension(1:8)     :: Flist4
        ...
        do k=1,dims%kmx-1
          do j=1,dims%jmx-1
            do i=1,dims%imx-1
              ...
              DQ0 = 0.0
              DQ1 = delQstar(i-1, j  , k  , 1:9)
              DQ2 = delQstar(i  , j-1, k  , 1:9)
              DQ3 = delQstar(i  , j  , k-1, 1:9)
              ...
              NewKminusFlux     = ScalarFlux(Q3, Q0, DQ3, Flist3)
              OldIminusFlux     = ScalarFlux(Q1, Q0, DQ0, Flist1)
              OldJminusFlux     = ScalarFlux(Q2, Q0, DQ0, Flist2)
              OldKminusFlux     = ScalarFlux(Q3, Q0, DQ0, Flist3)
              ...
              deltaU(1:9) = -residue(i,j,k,1:9) &
                - 0.5*((DelIminusFlux - LambdaTimesArea(1)*delQstar(i-1,j,k,1:9)) &
                     + (DelJminusFlux - LambdaTimesArea(2)*delQstar(i,j-1,k,1:9)) &
                     + (DelKminusFlux - LambdaTimesArea(3)*delQstar(i,j,k-1,1:9)) )

              delQstar(i,j,k,1:9) = deltaU(1:9)/D
            end do
          end do
        end do
        ...
        do k=dims%kmx-1,1,-1
              ...
              DQ0 = 0.0
              DQ4 = delQ(i+1, j  , k  , 1:9)
              DQ5 = delQ(i  , j+1, k  , 1:9)
              DQ6 = delQ(i  , j  , k+1, 1:9)
              ...
              NewKminusFlux     = ScalarFlux(Q6, Q0, DQ6, Flist6)
              OldIminusFlux     = ScalarFlux(Q4, Q0, DQ0, Flist4)
              OldJminusFlux     = ScalarFlux(Q5, Q0, DQ0, Flist5)
              OldKminusFlux     = ScalarFlux(Q6, Q0, DQ0, Flist6)
              ...
              delQ(i,j,k,1:9) = delQstar(i,j,k,1:9) &
                - 0.5*((DelIminusFlux - LambdaTimesArea(4)*delQ(i+1,j,k,1:9)) &
                     + (DelJminusFlux - LambdaTimesArea(5)*delQ(i,j+1,k,1:9)) &
                     + (DelKminusFlux - LambdaTimesArea(6)*delQ(i,j,k+1,1:9)) )/D
        ...
        do k=1,dims%kmx-1
          do j = 1,dims%jmx-1
            do i = 1,dims%imx-1
              ...
              conservativeQ(5) = (qp(i,j,k,5) / (gm-1.0)) + ( 0.5 * qp(i,j,k,1) * sum( qp(i,j,k,2:4)**2) )
              conservativeQ(6) = qp(i,j,k,1) * qp(i,j,k,6)
              conservativeQ(7) = qp(i,j,k,1) * qp(i,j,k,8)
              conservativeQ(8) = qp(i,j,k,1) * qp(i,j,k,8)
    ...
    end subroutine update_scalar_variables
    ...
    function ScalarFlux(ql, qr, du, inputs)
      ...
      real(wp), dimension(1:8)    , intent(in) :: inputs
      real(wp), dimension(1:n_var)             :: Flux
      real(wp), dimension(1:n_var)             :: ScalarFlux
      real(wp), dimension(1:n_var)             :: U ! conservative variables
      real(wp), dimension(1:n_var)             :: W ! new primitive variables
      real(wp), dimension(1:n_var)             :: P ! primitive variables of right cell
